...
*/
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            ///** @var ScolariteBundle\Entity\Seance em1 */
            ...
            if ( $sp < 3 and $sc < 2) {
                    ...
                    $check = $em1 ->CheckSalleSeance($s, $deb, $fin, $j);
                    if (!count($check)) {
                        $em = $this->getDoctrine()->getManager();
                        $em->persist($seance);
                        $em->flush();

                        return $this->redirectToRoute('seance_index', array('id' => $seance->getId()));
                    } else {
                        $this->addFlash(
                            'error', 'Cette salle est ocuppé à cet heure !.'
                        );
...
/*
    public function seanceEleveAction($id){
